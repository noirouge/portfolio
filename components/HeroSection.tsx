  ...
  return (
    <section className='h-screen flex flex-col justify-center items-center px-5'>
      ...
      <div className='flex flex-wrap gap-5 justify-center items-center' >
        <a href="https://drive.google.com/file/d/1kMV3bBcapKReA01ZQQp59MfPSGAvFeQV/view?usp=sharing" target="_blank" className="btn-danger px-2 w-44 h-13 font-bold">{t.buttons.resume}</a>
        <a href="#projects"  className="btn-empty px-2  w-44 h-13 font-bold">{t.buttons.projects}</a>
        <a href="#contact"  className="btn-empty px-2  w-44 h-13 font-bold">{t.nav.contact}</a>
      </div>
      <div className="flex gap-x-5 justify-center items-center h-20">
      ...
      <a href="#projects" className="absolute text-stone-700 dark:text-stone-400 bottom-10 flex flex-col items-center justify-center cursor-pointer hover:text-red-700 dark:hover:text-red-400 ">
        <span className="font-mono  text-[10px] tracking-[0.3em] uppercase">{t.hero.scroll}</span>
        <span className="inline-block h-10 w-px bg-linear-to-b from-red-600  via-red-500 to-transparent dark:from-red-500 dark:via-red-400"></span>
      </a>
    </section>
  )
}
